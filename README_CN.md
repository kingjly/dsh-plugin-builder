...
仓库现在包含三个严格按本 Skill 流程生成的插件。它们都已安装进本机 `web` profile，能在设置里检索，也都在真实 Web UI 中执行过。
...
在 **设置 → 插件 → 插件列表** 搜索 `showcase`，能看到三个插件均已挂载、已启用。

![DeepSeek Harness 插件列表搜索真实截图](./docs/images/plugin-inventory.png)

以上四张图均直接截取自本机 `http://127.0.0.1:3080` 的实时服务，没有生成或合成。

### 自动生成的验证报告

下面这张图与上述界面截图不同：它明确是根据命令输出自动生成的，数据来自 CLI 版本、三个静态校验、12 个 smoke tests 和已安装 profile 检查。
