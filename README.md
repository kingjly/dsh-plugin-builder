...
This is a real browser capture of the locally running `web` profile with all three showcase bundles installed. The first-run API-key dialog remains visible because this validation environment intentionally has no model credential configured.

![DeepSeek Harness Web UI running the showcase profile](./docs/images/deepseek-harness-web.png)

The screenshot is not generated or composited. Tool and hook plugins do not receive custom Web Settings cards automatically, so plugin-specific loading evidence is reported separately below.
