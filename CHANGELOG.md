...
- 将 `dsh-luna-pet` 升级为无外框桌宠：移除整块卡片背景、边框、阴影和模糊层，只保留 Luna 与独立控件。
- Luna 本体支持鼠标、触控笔和触屏拖拽；位置限制在可见视口内并保存到浏览器本地存储，刷新后自动恢复。
- 新增拖拽 bundle smoke test，并在真实 DSH Web 中完成“拖动 → 刷新 → 坐标不变”的验证；同步替换真实运行截图。
